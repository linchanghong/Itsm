...
public class AnalyzesServiceImpl implements IAnalyzesService {
    ...
    public String initProblemDatas(int queryMonth, int lines, int pageNum,
        ...
        //select t0.sysId sysId, t0.sysName sysName, t1.submitＣount submitCount, 
        //t3.acceptＣount acceptCount, t5.completedＣount completedCount 
        //from view_statistic4problem t0 left join (select t.sysid sysＩd, t.sysname sysＮame, count(t.submit) submitＣount 
        //from view_statistic4problem t where t.submitmonth = 9 group by t.sysid, t.sysname) t1 on t0.sysid = t1.sysＩd 
        //left join (select t2.sysid sysＩd, t2.sysname sysＮame, count(t2.accept) acceptＣount 
        //from view_statistic4problem t2 where t2.acceptmonth = 9 group by t2.sysid, t2.sysname) t3 on t1.sysＩd = t3.sysＩd 
        //left join (select t4.sysid sysＩd, t4.sysname sysＮame, count(t4.completed) completedＣount 
        //from view_statistic4problem t4 where t4.completedmonth = 9 group by t4.sysid, t4.sysname) t5 
        //on t1.sysＩd = t5.sysＩd order by sysId
