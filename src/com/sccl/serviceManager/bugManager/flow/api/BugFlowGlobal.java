package com.sccl.serviceManager.bugManager.flow.api;
...
import com.sccl.serviceManager.bugManager.flow.service.IBugFlowGlobalService;

@Component("bugFlowGlobal")
@RemotingDestination
public class BugFlowGlobal {
	private IBugFlowGlobalService flowGlobalService;

	public IBugFlowGlobalService getFlowGlobalService() {
		return flowGlobalService;
	}

	@Resource(name = "bugFlowGlobalService")
	public void setFlowGlobalService(IBugFlowGlobalService flowGlobalService) {
	...
	public String startFlowInstence(String json){
		...
		if(list.size()==4){
			try {
				...
				BigInteger billid = new BigInteger(String.valueOf(list.get(3)));
