...
@Component("bugFlowGlobalService")
public class BugFlowGlobalService implements IBugFlowGlobalService {
	...
	public List<FlowNodeInstence> startFlowInstence(MsPerson user, Long sendId,
			Long flowTypeId, BigInteger billid) throws FlowException {

		List<FlowNodeModelFull> resultlist = new ArrayList<FlowNodeModelFull>();

		// 获得流程实例
		BugFlowInstence flowInstence = (BugFlowInstence) dataManager.createQuery(
		        BugFlowInstence.class).queryWhere(
				"billID=" + billid + " and oprtTypeID=" + flowTypeId).single();
		...
		// 流程模拟执行初始化数据
		// 获得该业务的流程模型ID
		DataQuery query = dataManager.createQuery(
				FlowModel.class).queryWhere(
						"ISActive = '1' and OprtTypeID =" + flowTypeId
						+ " and CompId ="+ sender.getOrganization().getCompanyId());
			...
			if (fnmf.getNodeAttr() == -1) {
				...
				if (fnmf.getNextID() != null
					...
					if (tfOpertTypeID != 0) {// 需要省公司审批
						// 暂时不翻译此段代码
						 nodeArrList = addLeaderNode(nodeArrList, beginFNI, tfOpertTypeID, billid,
								 sender, flowmodel, endModleCode); // 添加省公司结点r
					}
		...
		try {
			...
			if ((resultSave == 0) && (resultInt != 0)) {
			    ...
			    BugFlowInstence fi = (BugFlowInstence) dataManager.createQuery(
			            BugFlowInstence.class).queryWhere(
						"Flowid='" + flowInstence.getFlowId() + "'").single();
				fi.setFlowState(136);
	...
	private void createFreshFlowNodeInstence(long flowId) {
		...
		String where = "FlowID = '"
				...
				+ flowId + "')";
		List<FlowNodeInstence> list = dataManager.createQuery(
				FlowNodeInstence.class).queryWhere(where).list();
	...
	private void setNextNodeId(List<FlowNodeModelFull> currentNodeList,
		...
		if (tempid!=null && tempid.equals("1")) {
			String andNodemark = fnm.getAndNodeMark();
			for (int i = len; i >= 0; i--) {
				FlowNodeModelFull full = currentNodeList.get(i);
				if (full.getApposeNodeIDs().equals("1") || full.getAndNodeMark() != andNodemark)
					break;
	...
	private boolean checkPrantDept(String flowDeptId, long sendDeptId) {
		...
		List<Organization> list = dataManager.createQuery(Organization.class)
				.queryWhere(
	...
	private List<FlowNodeModelFull> findNodePath(
			List<FlowNodeModelFull> nodeList, BigInteger billid, MsPerson sender)
			throws FlowException {
		...
		while (nextNodeAttr != -2) {
			...
			for (int i = 0; i < nodes.length; i++) {
				...
				case 4: // 自定义条件
					...
					String conditionStr = NodeMark.substring(fromnum + 2);//, (NodeMark.length() - fromnum - 2));
					...
					if (customDs != null) {
						if (customDs.size() >= 2) {
							...
							Map<String, Object> map = (Map<String, Object>) customDs.get(0);
							Set<String> key = map.keySet();
							if(key.size()!=1){
								throw new FlowException(nextNode.getNodeName() + "条件设置有误");
							}
							
							String selectResult = String.valueOf(map.get(key.iterator().next()));
							int meetCount = 0;
							for (String condition : conditionArr) {
								...
								String conditionvalue = condition.substring(0, index);
								String aimNode = condition.substring(index + 1, condition.length());
				...
				case 5: // 自定义条件
					...
					if (param.length > 0)
						andAutidFlag = (param[0].equals("true")) ? "false" : "true";

					List csDs = dataManager.nativeQuery(cssql);
					...
					if (csDs != null) {
						...
						for (int j = 0; j < csDs.size(); j++) {
							...
							if(map.size()==2){
								Object valueId = map.get("userId".toUpperCase());
								Object valueName = map.get("userName".toUpperCase());
								...
								custNode.setNodeNumber(nextNode.getNodeNumber() + "cin" + j);
								...
								custNode.setPointY(nextNode.getPointY() + (j * 10));
								custNode.setPreviewID(nextNode.getPreviewID());
								custNode.setFlowRoleID(Long.valueOf(userId));
								custNode.setFlowRoleName(userName);
								custNode.setDealWithDay(nextNode.getDealWithDay());
								custNode.setNextID(nextNode.getNextID());
								custNode.setIsNodeOneself(nextNode.getIsNodeOneself());
								custNode.setIsNeedCheckDate(nextNode.getIsNeedCheckDate());
								
								custNode.setApposeNodeIDs((csDs.size()>1?"1":"0"));
								customNextIds += "," + custNode.getNodeNumber();
								customrAndMark += custNode.getNodeNumber() + "|" + andAutidFlag + "&";
								customNodeList.add(custNode);
							}else
								 throw new FlowException(nextNode.getNodeName() +"人员查询设置有误");
							
						}
						if (param.length > 1) {
							String p2 = (param[1].equals("true")) ? "true" : "false";
							customrAndMark += "$" + p2;
						}
						nextIDs = nextNode.getNextID();
						nextNodeAttr = nextNode.getNodeAttr();
	 ...
	 * @return
	 */
	private List addLeaderNode(List<FlowNodeInstence> nodeArrList, FlowNodeInstence preNode,
			long flowTypeId, BigInteger billid, MsPerson sender, FlowModel flowmodel,
			String endModleCode) throws FlowException {
        ...
        //if (String.valueOf(compId).equals(sender.getOrganization().getCompanyId())) return nodeArrList;
        ...
        try
        {
        	flowModel = (FlowModel) dataManager.createQuery(FlowModel.class).queryWhere(
    				"ISActive = '1' and OprtTypeID ='" + flowTypeId+ "' and CompId ='"
    						+ compId+"'").single();
        	
        	if(flowModel==null) throw new FlowException("--省公司流程段流程模型未设置");
        	else
        ...
        for(FlowNodeModelFull flowNodeModel : thisFlowAllNodes) //遍历该模型中的所有节点
        {
            ...
            //}
            if (flowNodeModel.getNodeAttr() == 0 && flowNodeModel.getNodeMark().trim().equals(""))
            {
        ...
        //fj 2012-6-12
        //得到正确路径
        List<FlowNodeModelFull> nodeList = findNodePath(thisFlowAllNodes,billid, sender); //根据条件得到路径节点 
        //为避免重复修改nodeNumber;
        for(FlowNodeModelFull fnmf : nodeList){
            ...
            if (fnmf.getAndNodeMark()!=null && !fnmf.getAndNodeMark().equals(""))
            {
                ...
                for(String s : andnodemark)
                {
                    if (s==null || "".equals(s)) continue;
                    String nodenmber = s;
                    if (!s.equals("BeginNode") && !s.equals("EndNode") && s.indexOf("$") != 0)
                        nodenmber = "P-" + s;
                    andmark += ((andmark==null || andmark.equals(""))?"":"&") + nodenmber;
                }
        ...
        for (int i = 0; i < nodeList.size(); i++) //遍历该模型中的所有节点
        {
            ...
            if (flowNodeModel.getNodeAttr() == -2) continue;
                ...
                {
                    if (StrErr.equals(""))
                        StrErr = "--省公司-" + flowNodeModel.getFlowRoleName() +" 的管辖范围中未找到：" + flowUserName; //sender.UserName;
                    throw new FlowException(StrErr);
                }
                pfni.setCheckState(141);                   
                if (flag == 0)
                {
                    if (preNode.getNodeNumber().equals("BeginNode") && flowmodel.getIsSubmitModify()==false)
...
//        DataSet FlowRoleDS = new DataSet();
        ...
        String flowNodeModelstrSQL ="select a.*, case when a.NodeMark = '-10' then a.NodeMark else TO_CHAR(b.FlowRoleID) end as FlowRoleID,"
			+" case when a.NodeMark = '-10' then '项目经理(项目所属)' else b.FlowRoleName end as FlowRoleName from FlowNodeModel a"
+" Left Outer Join FlowRole b ON (case when regexp_like(a.NodeMark,'^\\d+$') then a.NodeMark else '' end) = b.FlowRoleID where a.FlowID = '"+flowModelID+"'";
		
        
        List<RoleAndNodeModel> flowNodeModelDS = (List<RoleAndNodeModel>)dataManager.nativeQuery(flowNodeModelstrSQL, RoleAndNodeModel.class);
        ...
        for(RoleAndNodeModel flowNodeModelDataRow : flowNodeModelDS){
         ...
         //   fnmf.PreviewID = FlowNodeModelDataRow["PreviewID"].ToString();
            //fj 处理天数据
            fnmf.setDealWithDay(flowNodeModelDataRow.getDealWithDay()==null?0:flowNodeModelDataRow.getDealWithDay());
            //节点独立
            fnmf.setIsNodeOneself(flowNodeModelDataRow.getIsNodeOneself()==null ? false : flowNodeModelDataRow.getIsNodeOneself());
            fnmf.setIsNeedCheckDate(flowNodeModelDataRow.getIsNeedCheckDate()==null ? false : flowNodeModelDataRow.getIsNeedCheckDate());
            if (flowNodeModelDataRow.getNodeAttr() == 0){
                //wmd 2009-09-29
                //foreach (DataRow FlowRoleRow in FlowRoleDS.Tables[0].Rows)
                //{
                //    if (FlowRoleRow["FlowRoleID"].ToString() == FlowNodeModelDataRow["NodeMark"].ToString())
                //    {
                //        fnmf.FlowRoleID = int.Parse(FlowRoleRow["FlowRoleID"].ToString());
                //        fnmf.FlowRoleName = FlowRoleRow["FlowRoleName"].ToString();
                //    }
                //}
                if (!"".equals(flowNodeModelDataRow.getFlowRoleID()))
                    fnmf.setFlowRoleID(Long.valueOf(flowNodeModelDataRow.getFlowRoleID()));
	...
	private FlowNodeInstence makeSureDoer(MsPerson sender,
			FlowNodeModelFull fnmf, FlowNodeInstence fni, BigInteger billid,
		...
		if (fnmf.getNodeMark().equals("-10")) {
			...
			FlowOprtType flowOprtType = (FlowOprtType) dataManager.createQuery(
					FlowOprtType.class).queryWhere(strSQLType).single();
			if (flowOprtType != null) {
				strSQLType = flowOprtType.getProjectManagerSQL();
				if (strSQLType != null && !strSQLType.equals("")) {
					if (strSQLType.contains("%d")) {
						strSQLType = strSQLType.replace("%d", String
								.valueOf(billid));
						List<MsPerson> ds = (List<MsPerson>) dataManager
								.nativeQuery(strSQLType, MsPerson.class);
						if (ds != null && ds.size() > 0) {
							fni.setHandlerId(Long.valueOf(ds.get(0)
									.getId()));
				...
				for (FlowRoleUsers flowRoleUsers : flowRoleUsersDs) {
					...
					if (managetype.equals("1")) { // 管辖部门
							...
							if (!"".equals(managedepart)
								...
								try {
									MsPerson cusUsers = (MsPerson) dataManager
											.findById(MsPerson.class, Integer
													.valueOf(handerid));
						...
						for (String manageUserId : manageUsers) {
							if (String.valueOf(senderUserId).equals(manageUserId)) {
								handerid = flowRoleUsers.getUserID();
								try {
						...
						for (String managecomp : manageComps) {
							if (!"".equals(managecomp)
									&& String.valueOf(senderCompId).equals(managecomp)) {
								handerid = flowRoleUsers.getUserID();
								try {
									handername = ((MsPerson) dataManager
											.findById(MsPerson.class, Integer
	 ...
	 * @throws FlowException 
	 */
	private void nodeOrderby(List<FlowNodeInstence> nodesArr, int reNodeCount) throws FlowException {
	 ...
	 * @throws FlowException 
	 */
	private int findNode(List<FlowNodeInstence> alNode, String nodeName) throws FlowException {
		int result = -1;
		String nodeStr = "";
		...
		if(result==-1) throw new FlowException("生成流程失败！");
		return result;
	}
	 ...
	 * @throws FlowException 
	 */
	private void setNodeOrder(List<FlowNodeInstence> alNodes, int numId) throws FlowException {
		nodeOrderId++;
	...
	public List<ViewFlowNodeInstence> getFlowNodeInstenceList(String typeId,
		...
		if(query!=null){
		    BugFlowInstence instence = (BugFlowInstence)query.single();
			list = getHistoryByFlowInsteceId(String
					.valueOf(instence.getFlowId()));
		}
		return list;
	}
	...
	public List<FlowNodeInstence> sendFlowTodo(ViewFlowApproval currentFni,
		...
		BugFlowInstence flowInstence = (BugFlowInstence) dataManager.findById(
		        BugFlowInstence.class, currentFni.getFlowId());
		...
		if (currentFni.getCheckState() == 145) {
			...
			// 转办
			 if (currentFni.getNodeAttr() != -2 && turnUser != null){
				 ...
				 FlowNodeInstence tempFlowNodeInstence =
				 this.nodesDStoObject(currentFni, false);
				 ...
				 if (isBrogther)
				 {
					 ...
					 {
						 FlowNodeInstence tempInstence = nodesDStoObject(brogherNode, false);
		...
		else if (currentFni.getCheckState() == 146) // 拒绝
		{
			...
			if (flowModel.getIsOneByOne()) // 逐级回退的情况
			{
				...
				MsPerson cusUsers = (MsPerson) dataManager.findById(
						MsPerson.class, Integer.valueOf(flowInstence
	...
	private boolean fniHasDone(ViewFlowApproval flowNodeInstence) {
		FlowNodeInstence fni = (FlowNodeInstence) dataManager.createQuery(
				FlowNodeInstence.class).queryWhere(
				"instId='" + flowNodeInstence.getInstId() + "'").single();
	...
	private FlowNodeInstence nodesDStoObject(ViewFlowApproval view, boolean bool) {
		...
		while (it.hasNext()) {
			Entry<String, JsonElement> entry = it.next();
			key = entry.getKey();
			jo.add(key.substring(0, 1).toLowerCase() + key.substring(1), entry
					.getValue());
		}
	...
	private FlowNodeInstence addNextNodeByTurnNode(ViewFlowApproval currentFni,
		...
		turnNode
				.setModuleCode(moduecode == null || moduecode.equals("") ? currentFni
						.getModuleCode()
						: moduecode);
	...
	private String getAndNodeArticle(ViewFlowApproval currentFni) {
		...
		String nodemarks = flowmark.substring(0, findex == -1 ? flowmark
				.length() : findex);
		return nodemarks;
	}
	...
	private List<ViewFlowApproval> getNextFNIlist(
			List<ViewFlowApproval> allflownodeinstence,
			ViewFlowApproval currentFni, boolean needAll) {
			...
			for (String nextid : currentFni.getNextids().split(",")) {
				for (ViewFlowApproval myflowTodoRow : allflownodeinstence) {
					...
					for (String nodeNumber : nodeNumbers) {
						...
						if (nodeNumber.equals(nextid) && (brotherAgreed || needAll)
								&& MakeRubbish == false) {
							...
							nextFlowNodeinstence.setTotalFlowId(currentFni
									.getTotalFlowId());
							nextFlowNodeinstence.setBillId(currentFni.getBillId());
	...
	private Boolean getAndNodeIsCompetition(ViewFlowApproval currentFni) {
		String flowmark = currentFni.getOther1();
		int findex = flowmark.indexOf("$");
		String math = flowmark.substring(findex + 1, findex == -1 ? 0
				: flowmark.length() - findex - 1);
		if (math.trim() == "true") // 如果为真将未审批的兄弟节点都审批通过
			return true;
		return false;
	}
...
//			FlowNodeInstence flowNodeInstence = (FlowNodeInstence) dataManager
...
//			}
			...
			list = dataManager.createQuery(FlowNodeInstence.class).queryWhere(
					where).list();
...
//			}
			...
			BugFlowInstence flowInstence = (BugFlowInstence) dataManager.findById(
			        BugFlowInstence.class, Long.valueOf(flowId));
	...
	private void flowUntreadtoBegin(long flowId, long instId,
		...
		List<FlowNodeInstenceNew> newList = dataManager.createQuery(
				FlowNodeInstenceNew.class)
