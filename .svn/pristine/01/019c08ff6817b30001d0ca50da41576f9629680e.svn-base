...
//		return knowledgeInfoService.addKnowledgeInfo(knowledgeInfoJson);
    ...
    @Produces(value = {"application/json"})
	public String findAllKnowledgeInfoPage(@PathParam(value = "sqlWhere") String sqlWhere,@PathParam(value = "first") int first,
			@PathParam(value = "size") int size, @PathParam(value = "isCount") boolean isCount){
		return knowledgeInfoService.findAllKnowledgeInfoPage(sqlWhere,first,size,isCount);
	}
