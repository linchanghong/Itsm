...
public class FlowRoleService implements IFlowRoleService {
	...
	public List<FlowRole> getAllRole(String where, String orderby){
		if(where==null || where.equals("")) where="1=1";
		if(orderby==null || orderby.equals("")) orderby="FlowRoleID asc";
		String sql="select * from FLOWROLE t where "+where+" order by "+orderby;
		//String sql="select * from VIEW_FLOWROLEUSERS t order by "+orderby;
		
		List<FlowRole> list = (List<FlowRole>)dataManager.nativeQuery(sql, FlowRole.class);
		return list;
	}
}
