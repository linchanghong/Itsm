...
//    }
            ...
            if (subDemandID != null && subDemandID.intValue() > 0) {
                ...
                if (status > 1){
                    if(status == 2)
                        ...
                        addServiceOrderBySubDemandID(subDemandID);
                    }else 
                        sql += " ";
                }
                if (status == 1)
    ...
    /**
     * 
     * @Title: addServiceOrderBySubDemandID
     * @Description:根据一个子需求对象id，向数据库存入一个ServiceOrder对象
     * @param subDemandID 
     * void        返回类型
     * @throws
     */
    private void addServiceOrderBySubDemandID(Long subDemandID){
        SubDemand apply = dataManager.findById(SubDemand.class, subDemandID);
        ServiceOrder order;
        if(apply != null){
            //0:bug、问题；1：子需求
            order = BugManagerUtil.createServiceOrder(apply, 1);
        ...
        try {
            ...
            List lst = dataManager.createNativeQuery(
                    "select t.user_id as userId " + "from t_system_person t "
                            + "where t.user_status = 201 " + "and t.user_id = "
